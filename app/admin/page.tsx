...
export default async function AdminPage() {
  ...
  return <div><div className="portal-heading"><div><p className="eyebrow">Painel administrativo</p><h1 className="display-font">Olá, {user.name.split(" ")[0]}.</h1><p>Uma visão simples da agenda e dos retornos pendentes.</p></div><Link className="button-primary" href="/admin/agenda">Abrir agenda</Link></div><div className="dashboard-grid"><Link href="/admin/agenda" className="data-card"><span>Solicitações pendentes</span><strong>{pending}</strong><small>Precisam de decisão</small></Link><Link href="/admin/agenda" className="data-card"><span>Consultas hoje</span><strong>{today}</strong><small>Compromissos confirmados</small></Link><Link href="/admin/mensagens" className="data-card"><span>Mensagens de pacientes novas</span><strong>{internalUnread}</strong><small>Contato assíncrono interno</small></Link><Link href="/admin/mensagens" className="data-card"><span>Contatos da homepage novos</span><strong>{homepageUnread}</strong><small>Formulários públicos não lidos</small></Link></div><section className="portal-panel"><div className="panel-heading"><div><p className="eyebrow">Homepage</p><h2 className="display-font">Card de frase</h2></div></div><HomepageQuoteSettingsForm settings={quoteSettings} /></section></div>;
}
